...
type MobileSectionKey = 'shop' | 'discover' | 'information' | 'contact' | 'newsletter'

export function Footer({ onNavigate, onShowToast }: FooterProps) {
  const [openFaq, setOpenFaq] = useState<string | null>(null)
  const [openMobileSection, setOpenMobileSection] = useState<MobileSectionKey | null>(null)
  ...
  const mobileSections: Array<{ key: MobileSectionKey; title: string }> = [
    { key: 'shop', title: 'Shop' },
    { key: 'discover', title: 'Discover' },
    { key: 'information', title: 'Information' },
    { key: 'contact', title: 'Contact / Social' },
    { key: 'newsletter', title: 'Join the Janasya Circle' },
  ]
  ...
  const renderFooterLinks = (items: string[], groupTitle: string) => (
    <ul className="space-y-3">
      {items.map((item) => (
        <li key={item}>
          <button
            type="button"
            onClick={() => handleFooterLink(item, groupTitle)}
  ...
  const renderMobileAccordionContent = (key: MobileSectionKey) => {
    if (key === 'shop') return renderFooterLinks(FOOTER_SHOP, 'Shop')
    if (key === 'discover') return renderFooterLinks(FOOTER_DISCOVER, 'Discover')
    if (key === 'information') return renderFooterLinks(FOOTER_INFO, 'Information')

    if (key === 'contact') {
      return (
        <div className="space-y-4">
          {renderFooterLinks(['A-15/5-A, Road No 8, Udhana, Surat-394210', '+91-9310070073', '+91-7837796109 (WhatsApp)', 'care@janasya.com'], 'Contact')}

          <div>
            <p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.22em] text-[#8a7f73]">Accepted Payments</p>
            <div className="flex flex-wrap gap-2">
              {paymentMethods.map((method) => (
                <div
                  key={method}
                  className="rounded-full border border-[#d9d1c2] bg-[#f8f4ec] px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.24em] text-[#4c4a46]"
                >
                  {method}
                </div>
              ))}
    ...
    return (
      <form onSubmit={handleNewsletterSubmit} className="space-y-3">
        <p className="text-[13px] leading-6 text-[#5f5a55]">
        ...
        <input
          ...
          aria-describedby={newsletterStatus !== 'idle' ? 'newsletter-feedback-mobile' : undefined}
        />
        <button
          type="submit"
          className="interactive-btn inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#0ea5a4] px-6 py-3 text-[12px] font-semibold uppercase tracking-[0.24em] text-white transition-all duration-300 hover:-translate-y-0.5 hover:bg-[#0c8f90] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
          Subscribe
          <ArrowRight size={14} strokeWidth={2} />
        </button>
        {newsletterStatus === 'success' && (
          <p id="newsletter-feedback-mobile" className="text-sm text-[#0ea5a4]" role="status">
            You&apos;re subscribed. Welcome to the Janasya Circle.
          </p>
        )}
        {newsletterStatus === 'error' && (
          <p id="newsletter-feedback-mobile" className="text-sm text-red-500" role="alert">
    ...
    <footer className="border-t border-gray-100 bg-cream pb-6 pt-12 sm:pt-14">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="lg:hidden">
          <section className="rounded-[28px] border border-[#e5ddd2] bg-white/80 px-4 py-5 shadow-[0_14px_40px_rgba(15,23,42,0.04)] sm:px-5">
            <div className="mb-4 text-center sm:mb-5">
              <span className="text-3xl font-semibold text-brand" style={{ fontFamily: "'Noto Serif Devanagari', serif" }}>
                जनस्या
              </span>
              <p className="mt-2 text-[11px] uppercase tracking-[0.16em] text-[#7a7266] sm:text-[12px]">
                Modern Indian Wear • Crafted for Everyday Elegance
              </p>
            </div>

            <div className="space-y-2.5">
              <div className="overflow-hidden rounded-3xl border border-[#e8dfd0] bg-[#fbf8f2] shadow-[0_6px_16px_rgba(15,23,42,0.03)]">
                <button
                  type="button"
                  onClick={() => setOpenFaq((prev) => (prev === 'faq' ? null : 'faq'))}
                  aria-expanded={openFaq === 'faq'}
                  className="interactive-btn flex min-h-12 w-full items-center justify-between gap-3 px-4 py-3.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                >
                  <span className="text-[14px] font-semibold leading-none text-brand">Frequently Asked Questions</span>
                  <span
                    className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#d9d1c2] bg-white text-brand transition-transform duration-300 hover:bg-[#faf8f5] active:scale-95 ${openFaq === 'faq' ? 'rotate-180' : ''}`}
                  >
                    <ChevronDown size={16} strokeWidth={1.8} />
                  </span>
                </button>
                <div
                  className={`grid transition-all duration-300 ease-out ${openFaq === 'faq' ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0 pointer-events-none'}`}
                  aria-hidden={openFaq !== 'faq'}
                >
                  <div className="min-h-0 overflow-hidden px-4 pb-4">
                    <div className="space-y-2">
                      {faqItems.map((item) => {
                        ...
                        return (
                          <div key={item.key} className="overflow-hidden rounded-[18px] border border-[#ece4d8] bg-white">
                            <button
                              type="button"
                              onClick={() => setOpenFaq(isOpen ? 'faq' : item.key)}
                              aria-expanded={isOpen}
                              className="interactive-btn flex min-h-11 w-full items-center justify-between gap-3 px-4 py-3 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                            >
                              <span className="text-[13px] font-medium leading-snug text-brand">{item.question}</span>
                              <span
                                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#d9d1c2] bg-white text-brand transition-transform duration-300 hover:bg-[#faf8f5] active:scale-95 ${isOpen ? 'rotate-180' : ''}`}
                              >
                            ...
                            >
                              <div className="min-h-0 overflow-hidden">
                                <p className="px-4 pb-4 pt-1 text-[13px] leading-6 text-[#5a5a5a]">{item.answer}</p>
                              </div>
                            </div>
                          </div>
                        )
              ...
              {mobileSections.map((section) => {
                ...
                return (
                  <div key={section.key} className="overflow-hidden rounded-3xl border border-[#e8dfd0] bg-[#fbf8f2] shadow-[0_6px_16px_rgba(15,23,42,0.03)]">
                    <button
                      type="button"
                      ...
                      className="interactive-btn flex min-h-12 w-full items-center justify-between gap-3 px-4 py-3.5 text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent"
                    >
                      <span className="text-[14px] font-semibold leading-none text-brand">{section.title}</span>
                      <span
                        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-[#d9d1c2] bg-white text-brand transition-transform duration-300 hover:bg-[#faf8f5] active:scale-95 ${isOpen ? 'rotate-180' : ''}`}
                      >
                        <ChevronDown size={16} strokeWidth={1.8} />
                    ...
                    <div
                      ...
                      aria-hidden={!isOpen}
                    >
                      <div className="min-h-0 overflow-hidden px-4 pb-4 pt-1">{renderMobileAccordionContent(section.key)}</div>
                    </div>
                  </div>
                )
              })}
            ...
            <div className="mt-5 rounded-3xl border border-[#e2d9c9] bg-white/80 p-4 text-center shadow-[0_10px_24px_rgba(15,23,42,0.04)]">
              <h3 className="text-[14px] font-medium text-brand" style={{ fontFamily: "'Jost', sans-serif" }}>
                Follow Our Journey
              </h3>
            ...
            <div className="mt-5 border-t border-[#e7dfd1] pt-4 text-center text-[11px] uppercase tracking-[0.16em] text-[#786f64]">
              © 2026 Janasya. All rights reserved.
            </div>
          </section>
        </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 xl:gap-5">
              {footerLinkGroups.map((group) => (
                <div key={group.title} className="rounded-[22px] border border-[#e6ddd1] bg-white/70 p-4 sm:p-5">
                  ...
                  {renderFooterLinks(group.items, group.title)}
                </div>
              ))}
            </div>
